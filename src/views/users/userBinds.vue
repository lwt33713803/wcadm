    ...
    <n-table :bordered="false" :single-line="false" striped>
      ...
      <tbody>
        <tr v-for="(title, index) in tableData" :key="index">
          ...
          <td>{{ title.name }}</td>
          <td><n-image width="80" :src="title.headImg" /></td>
          <td>{{ title.sex }}</td>
          <td>{{ title.mobile }}</td>
          <td>{{ title.createdTime }}</td>
          <td>{{ title.channelCode }}</td>
          <td>{{ title.shareNo }}</td>
          <td>{{ title.isRechange }}</td>
        </tr>
      </tbody>
      <tfoot>
        <tr>
...
import { usePostTable } from './datas/userBindsTable'
...
const { loading, columns, tableData, initColumns, initTableData, getTableData, page } = usePostTable()
